...
#### 전략 3: 재귀 없는 DFS (🤖)
- 현재 인덱스 i와 현재 합을 같이 기록함
- 스택에 튜플을 2번 추가
    - 다음 인덱스 및 현재 합에 i번째 숫자를 더한 값으로 이루어진 튜플
    - 다음 인덱스 및 현재 합에 i번째 숫자를 뺀 값으로 이루어진 튜플
- i가 numbers의 길이와 같아지면,
    - 현재 합이 target이면 카운트를 1 증가
    - 끝에 도달했으므로 스택에 더 이상 추가하지 않음
...
### DP 계열
#### 전략 7 (🗒️)
- 초기값을 0으로 두고, 처음부터 i번째 수까지 사용해서 특정 숫자를 만들 수 있는 가짓수를 기록
    - 첫 번째 수만을 사용해서 특정 수를 만들 수 있는 가짓수는
        - 0에 첫번째 수를 더함: 1가지
        - 0에 첫번째 수를 뺌: 1가지
- 기록을 위해 2차원 배열을 사용
    - 바깥 배열이 i, 안쪽 배열이 특정 숫자를 의미
- 첫 번째 수부터 마지막 수까지 사용해서 타겟 수를 만들 수 있는 가짓수를 반환

### BFS 계열
#### 전략 8 (🗒️)
- 재귀 없는 DFS의 전략에서 스택 대신 큐를 사용